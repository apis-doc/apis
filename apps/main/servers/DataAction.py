...
class DataActionView(object):
    def __init__(self, request, dao: Dao, unique_fs=None):
        ...
        self._dao = dao
        self.pk_name = self._dao.pk_name()
        self.unique_fs = unique_fs or tuple()

    def is_repeat(self, **fields) -> list:
        fields = fields or self._dao.verbose_names(*self.unique_fs)
        err_msg = []
        for field_name in fields.keys():
            value = self.params.get(field_name)
            pks = self._dao.query({field_name: value}).values_list(self.pk_name, flat=True)
    ...
    def delete(self, obj_id=None, range_query=None, **_) -> dict:
        ...
        objs = self._dao.query({self.pk_name: obj_id})
        if not objs:
        ...
        if hasattr(obj, 'delete_time'):
            ...
            self._dao.manage.filter(**self.params).delete()
        return dict()
    ...
    def create(self, **_) -> dict:
        for fun in [self.is_repeat, self.exist_foreign]:
            fun()
        try:
            self._dao.manage.create(**self._dao.params(self.params))
    ...
    def detail(self, **_) -> dict:
        ...
        objs = self._dao.query({self.pk_name: pk_value})
        if not objs:
    ...
    def update(self, range_query=None, **_) -> dict:
        ...
        try:
            ...
            self._dao.query(filters).update(**self._dao.params(self.params))
        except IntegrityError as e:
            self.db_error(e)
        return dict()
    ...
    def db_error(self, e: Exception):
        ...
        if err_msg.find('1062') > 0 and err_msg.find('Duplicate') > 0 and err_msg.find('PRIMARY') > 0:
            pk_field_name = self._dao.verbose_names(self.pk_name).get(self.pk_name)
            raise ClientErr(msg=f'主键标识({pk_field_name})重复, 请核对后重试')
        raise UnknownErr(msg='插入数据异常，请稍后重试', detail=err_msg)
...
# class UserDataActionView(DataActionView):
#
#     def __init__(self, request, ):
#         super().__init__(request, user_dao)
#
#     def exist_foreign(self, **fields):
#         err_msg = []
#         role_id = self.params.get('role_id')
#         if role_id:
#             if not role_dao.query({'id': role_id}).exists():
#                 err_msg.append(f'关联角色({role_id})不存在')
#         det_site_id = self.params.get('det_site_id')
#         if det_site_id:
#             if not det_dao.query({'id': det_site_id}).exists():
#                 err_msg.append(f'关联检测站({det_site_id})不存在')
#         if err_msg:
#             raise ClientErr(msg=f'{"、".join(err_msg)}不存在')
